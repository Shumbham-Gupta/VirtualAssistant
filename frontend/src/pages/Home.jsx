...
function Home() {
  ...
  const [isAwake,setIsAwake]=useState(false); // ✅ NEW

  const isSpeakingRef=useRef(false);
  const isRecognizingRef=useRef(false);
  const recognitionRef=useRef(null);
  const lastInteractionRef=useRef(Date.now()); // ✅ NEW
  ...
  const handleCommand = (data) => {
    ...
    speak(response); // ✅ FIXED (only once)
  ...
  useEffect(()=>{
    ...
    recognition.onresult= async (e)=>{
      ...
      // 🟢 Wake up
      if(!isAwake && lowerText.includes(wakeWord)){
        setIsAwake(true);
      ...
      // 🔴 Sleep command
      if(lowerText.includes("stop") || lowerText.includes("sleep")){
        setIsAwake(false);
    ...
    // ⏳ Auto sleep after 20 sec
    const interval = setInterval(()=>{
      if(isAwake && Date.now() - lastInteractionRef.current > 20000){
        setIsAwake(false);
        speak("Going idle");
      }
    },5000);
    ...
    setTimeout(() => {
      if (userData?.assistantName) {
        const greeting = new SpeechSynthesisUtterance(
          `Hello, I'm ${userData.assistantName}. Say my name to wake me up.`
        );
        greeting.lang = "hi-IN";
        window.speechSynthesis.speak(greeting);
      }
  ...
  return (
    <div className="w-full min-h-screen bg-gradient-to-t from-black to-blue-900 flex flex-col justify-center items-center gap-6 px-4 relative">
      ...
      <div className="absolute top-4 right-4">
        ...
        <div className="hidden sm:flex flex-col gap-2">
          <button className="bg-white px-4 py-1 rounded-full" onClick={handleLogout}>
            Log Out
          </button>

          <button className="bg-white px-4 py-1 rounded-full" onClick={() => navigate("/customize")}>
            Customize
          </button>
        </div>
      </div>

      <div className="w-[100px] h-[120px] sm:w-[130px] sm:h-[160px] md:w-[190px] md:h-[240px] rounded-3xl overflow-hidden border-2 border-white">
      ...
      {/* 🔥 STATUS */}
      <p className={`text-sm ${isAwake ? "text-green-400" : "text-red-400"}`}>
        {isAwake ? "Listening..." : "Sleeping"}
      </p>

      {!aiText ? (
        <img src={userImg} className="w-[120px]" />
      ) : (
        <img src={aiImg} className="w-[120px]" />
      )}
